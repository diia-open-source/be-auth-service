...
import { DiiaIdSharingBarcodeStrategyService } from '@services/userAuthSteps/strategies'
...
describe('DiiaIdSharingBarcodeStrategyService', () => {
    ...
    describe('method: `verify`', () => {
        it.each([
            ...
            async (method: AuthMethod, options: AuthStrategyVerifyOptions) => {
                ...
                const diiaIdSharingBarcodeStrategyService = new DiiaIdSharingBarcodeStrategyService(userAuthTokenService)

                jest.spyOn(userAuthTokenService, 'prepareUserData').mockResolvedValueOnce(user)

                await expect(async () => {
                    await diiaIdSharingBarcodeStrategyService.verify(options)
        ...
        it(`should successfully verify ${AuthMethod.PhotoId} auth method`, async () => {
            ...
            expect(await diiaIdSharingBarcodeStrategyService.verify(options)).toEqual([])
            ...
            (expectedProcessCode: ProcessCode, inputStatus: UserAuthStepsStatus, inputStep: UserAuthStep) => {
                expect(
                    processCodeDefinerService.getProcessCodeOnVerify(
                        inputStatus,
                        inputStep,
                        diiaIdSharingBarcodeStrategyService.authStepsStatusToAuthMethodProcessCode,
                    ),
