...
import { DiiaIdSharingDeeplinkStaticStrategyService } from '@services/userAuthSteps/strategies'
...
describe('DiiaIdSharingDeeplinkStaticStrategyService', () => {
    ...
    describe('method: `verify`', () => {
        it.each([
            ...
            async (method: AuthMethod, options: AuthStrategyVerifyOptions) => {
                ...
                const diiaIdSharingDeeplinkStaticStrategyService = new DiiaIdSharingDeeplinkStaticStrategyService(userAuthTokenService)

                jest.spyOn(userAuthTokenService, 'prepareUserData').mockResolvedValueOnce(user)

                await expect(async () => {
                    await diiaIdSharingDeeplinkStaticStrategyService.verify(options)
        ...
        it(`should successfully verify ${AuthMethod.PhotoId} auth method`, async () => {
            ...
            expect(await diiaIdSharingDeeplinkStaticStrategyService.verify(options)).toEqual([])
            ...
            (expectedProcessCode: ProcessCode, inputStatus: UserAuthStepsStatus, inputStep: UserAuthStep) => {
                expect(
                    processCodeDefinerService.getProcessCodeOnVerify(
                        inputStatus,
                        inputStep,
                        diiaIdSharingDeeplinkStaticStrategyService.authStepsStatusToAuthMethodProcessCode,
                    ),
