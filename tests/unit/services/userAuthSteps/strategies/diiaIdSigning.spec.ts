...
import { DiiaIdSigningStrategyService } from '@services/userAuthSteps/strategies'
...
describe('DiiaIdSigningStrategyService', () => {
    ...
    describe('method: `verify`', () => {
        it(`should successfully verify ${AuthMethod.PhotoId} auth method`, async () => {
            ...
            const diiaIdSigningStrategyService = new DiiaIdSigningStrategyService(userAuthTokenService)

            jest.spyOn(userAuthTokenService, 'prepareUserData').mockResolvedValueOnce(user)

            expect(await diiaIdSigningStrategyService.verify(options)).toEqual([])
        ...
        it.each([
            ...
            async (method: AuthMethod, options: AuthStrategyVerifyOptions) => {
                ...
                await expect(async () => {
                    await diiaIdSigningStrategyService.verify(options)
            ...
            (expectedProcessCode: ProcessCode, inputStatus: UserAuthStepsStatus, inputStep: UserAuthStep) => {
                expect(
                    processCodeDefinerService.getProcessCodeOnVerify(
                        inputStatus,
                        inputStep,
                        diiaIdSigningStrategyService.authStepsStatusToAuthMethodProcessCode,
                    ),
