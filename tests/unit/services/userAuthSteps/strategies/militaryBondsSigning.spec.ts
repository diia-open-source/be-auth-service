...
import { MilitaryBondsSigningStrategyService } from '@services/userAuthSteps/strategies'
...
describe('MilitaryBondsSigningStrategyService', () => {
    ...
    describe('method: `verify`', () => {
        it.each([
            ...
            async (method: AuthMethod, options: AuthStrategyVerifyOptions) => {
                const militaryBondsSigningStrategyService = new MilitaryBondsSigningStrategyService(userAuthTokenService)

                jest.spyOn(userAuthTokenService, 'prepareUserData').mockResolvedValueOnce(user)

                await expect(async () => {
                    await militaryBondsSigningStrategyService.verify(options)
        ...
        it(`should successfully verify ${AuthMethod.PhotoId} auth method`, async () => {
            ...
            expect(await militaryBondsSigningStrategyService.verify(options)).toEqual([])
            expect(userAuthTokenService.prepareUserData).toHaveBeenCalledWith({ method, requestId, headers })
        })
            ...
            (expectedProcessCode: ProcessCode, inputStatus: UserAuthStepsStatus, inputStep: UserAuthStep) => {
                expect(
                    processCodeDefinerService.getProcessCodeOnVerify(
                        inputStatus,
                        inputStep,
                        militaryBondsSigningStrategyService.authStepsStatusToAuthMethodProcessCode,
                    ),
